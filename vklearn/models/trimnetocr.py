...
class TrimNetOcr(OCR):
    ...
    def calc_score(
        ...
        # score = 0.
        # pred  = inputs.argmax(dim=2).cpu().numpy()
        # pred  = map(lambda x: (np.convolve(x, [-1, 1], mode='same') != 0) * x, pred)
        # true  = targets.cpu().numpy()
        # if target_lengths is not None:
        #     lengths = target_lengths.cpu().numpy()
        # else:
        #     lengths = np.full(targets.shape[0], targets.shape[1], dtype=np.int64)
        # for pred_line, true_line, length in zip(pred, true, lengths):
        #     pred_line = np.array([num for num in pred_line if num > 0])
        #     length_min = min(pred_line.size, length)
        #     length_max = max(pred_line.size, length)
        #     score += (pred_line[:length_min] == true_line[:length_min]).sum() / length_max
        # score /= true.shape[0]
        # return float(score)
